...
const Profile = () => {
    ...
    const { userData, token, fetchUserData, loading, CircularProgress } = useContext(AppContext)
    const [edit, setEdit] = useState(false)
    const [profile_loading, setProfile_loading] = useState(false)
                ...
                <div className='flex flex-col max-w-xl'>
...

                    {
                        edit ? <button type='submit' onClick={(e) => { setEdit(false); submitHandler(e) }} className='outline-0 cursor-pointer px-12 mt-8 py-2 bg-[#4fd19f] rounded-full text-white w-fit'>
                            Save information
                        </button> : profile_loading ? <div className='px-12 mt-8 py-2 border border-[#4fd19f] flex items-center justify-center rounded-full w-fit'>
                            <div role="status">
