  ...
  return (
    <div className='min-h-screen flex items-center justify-center'>
      <form onSubmit={(e) => handleSubmit(e)} className='h-fit w-full max-w-96 text-sm text-zinc-600 p-8 rounded-xl shadow-lg border border-zinc-200 flex flex-col items-start justify-center gap-4'>
        ...
        <button disabled={loading} type="submit" className='w-full cursor-pointer outline-0 py-2 text-base text-center flex items-center justify-center text-white bg-[#4fd19f] rounded-sm'>{!loading ? 'Login' :
          <div role="status">
        ...
        <p className=''>{adminLogin ? 'Want to login as doctor? ' : 'Want to login as admin ? '} <span className='underline text-blue-500 cursor-pointer' onClick={() => setAdminLogin(!adminLogin)}>{adminLogin ? 'Doctor login' : 'Admin login'}</span></p>
      </form>
    </div>
  )
}
